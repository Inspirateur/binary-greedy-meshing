...
fn fast_mesh_opaque(c: &mut Criterion) {
    ...
    c.bench_function("fast_mesh_opaque", |b| b.iter(|| {
        mesher.clear();
...
fn mesh_opaque(c: &mut Criterion) {
    ...
    c.bench_function("mesh_opaque", |b| b.iter(|| {
        mesher.clear();
...
fn fast_mesh_transparent(c: &mut Criterion) {
    ...
    c.bench_function("fast_mesh_transparent", |b| b.iter(|| {
        mesher.clear();
        mesher.fast_mesh(black_box(voxels.as_slice()), black_box(&opaque_mask), black_box(&trans_mask));
    }));
}
...
fn mesh_transparent(c: &mut Criterion) {
    ...
    let mut mesher = bgm::Mesher::new();
    let transparent_blocks = BTreeSet::from([2]);
    c.bench_function("mesh_transparent", |b| b.iter(|| {
        mesher.clear();
